...
class DP2(BasePolicy):
    ...
    def get_local_agent_from_batch(self, batch, agent_id):
        ...
        - Concatenate all camera views side by side (along width dimension)
        - Use only this agent's own state
        """
        agent_obs = {}
        
        # Collect all camera views and concatenate them side by side
        all_head_cams = []
        for other_agent_id in range(self.agent_num):
            cam_key = f'head_cam_{other_agent_id}'
            if cam_key in batch:
                all_head_cams.append(batch[cam_key])
        
        # Concatenate cameras side by side along width dimension: [B, T, 3, H, W] -> [B, T, 3, H, W*num_agents]
        if all_head_cams:
            # Concatenate along width dimension (dim=-1, which is the W dimension)
            # Shape: [B, T, 3, 240, 320] + [B, T, 3, 240, 320] -> [B, T, 3, 240, 640]
            stacked_cam = torch.cat(all_head_cams, dim=-1)  # [B, T, 3, H, W*num_agents]
            agent_obs['head_cam'] = stacked_cam
