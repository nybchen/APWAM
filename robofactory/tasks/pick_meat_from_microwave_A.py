...
@register_env("PickMeatFromMicrowaveA-rf", max_episode_steps=500)
class PickMeatFromMicrowaveAEnv(BaseEnv):
    """
    Three robots task for picking meat from microwave with active perception - Mode A.
    ...
    Mode A: Microwave at original position [0, 0., 0.26]
    """
    ...
    def __init__(
        ...
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'pick_meat_from_microwave_A.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        # Position meat inside the microwave interior
    ...
    def picker_agent(self) -> PandaWristCam:
        """Robot 2: Pick meat and place to goal"""
    ...
    def evaluate(self):
        ...
        # Check if meat is on the table (not in microwave)
