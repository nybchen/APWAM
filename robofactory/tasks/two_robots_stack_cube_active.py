...
@register_env("TwoRobotsStackCubeActive-rf", max_episode_steps=500)
class TwoRobotsStackCubeActiveEnv(BaseEnv):
    ...
    def __init__(
        ...
        # print(f"SUPPORTED_ROBOTS: {self.SUPPORTED_ROBOTS}")
        # print(f"robot_uids: {robot_uids}")
        if 'config' in kwargs:
    ...
    def _default_sensor_configs(self):
        ...
        for agent in self.agent.agents:
            ...
            temp += 1
        # print(f"all_camera_configs: {all_camera_configs}")
    ...
    def debug_robot_sensors(self):
        print("=== ROBOT DEBUG ===")
        print(f"Agent type: {type(self.agent)}")
        print(f"Agent agents: {self.agent.agents if hasattr(self.agent, 'agents') else 'No agents'}")
        
        # Check each agent
        if hasattr(self.agent, 'agents'):
            for i, agent in enumerate(self.agent.agents):
                print(f"Agent {i}: {type(agent)}")
                print(f"Agent {i} sensor configs: {agent._sensor_configs}")
                print(f"Agent {i} has _sensor_configs: {hasattr(agent, '_sensor_configs')}")