...
@register_env("PickMeatFromMicrowaveC-rf", max_episode_steps=500)
class PickMeatFromMicrowaveCEnv(BaseEnv):
    """
    Three robots task for picking meat from microwave with active perception - Mode C.
    ...
    Mode C: Microwave at position [-0.3, 0.3, 0.26]
    """
    ...
    def __init__(
        ...
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'pick_meat_from_microwave_C.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
