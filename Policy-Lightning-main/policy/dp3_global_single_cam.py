"""
DP3GlobalSingleCam: All agents use ONE specified pointcloud + shared UNet.

- Observation: All agents see only pointcloud from one specified source (e.g. pointcloud_0) + own state
- Model: Single UNet predicts joint action trajectory for all agents
"""
...
class DP3GlobalSingleCam(BasePolicy):
    """
    Multi-agent DP3 variant that:
    - All agents see only ONE specified agent's pointcloud (pointcloud_source_id) + own state
    ...
    def __init__(
        ...
        # Which pointcloud source all agents use (e.g. 0 -> pointcloud_0)
        pointcloud_source_id: int = 0,
        **kwargs,
    ):
        ...
        # Note: obs_dict pointcloud shape is per-camera; we will concatenate all in get_local_agent_from_batch
        ...
        cprint(f"[DP3GlobalSingleCam] use_pc_color: {self.use_pc_color}", "yellow")
        cprint(f"[DP3GlobalSingleCam] pointnet_type: {self.pointnet_type}", "yellow")
        ...
        self.pointcloud_source_id = pointcloud_source_id
        cprint(f"[DP3GlobalSingleCam] pointcloud_source_id: {self.pointcloud_source_id}", "yellow")

    def get_local_agent_from_batch(self, batch: Dict[str, torch.Tensor], agent_id: int):
        """
        Get observations for an agent.
        - All agents use only pointcloud from pointcloud_source_id
        - Use only this agent's own state
        """
        agent_obs = {}

        pc_key = f"pointcloud_{self.pointcloud_source_id}"
        if pc_key in batch:
    ...
    def _encode_global_obs_features(
        ...
        for agent_id in range(self.agent_num):
            ...
            if len(agent_nobs) == 0:
                raise RuntimeError(
                    f"No observation keys found for agent {agent_id} in DP3GlobalSingleCam."
                )
            obs_encoder = (
        ...
        global_features = torch.cat(per_agent_features, dim=-1)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if sample_agent_nobs is None:
            raise RuntimeError("DP3GlobalSingleCam.predict_action: no agent observations found.")
    ...
    def compute_loss(self, batch):
        ...
        nactions_all = torch.cat(nactions_list, dim=-1)
