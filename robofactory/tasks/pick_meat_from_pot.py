...
@register_env("PickMeatFromPot-rf", max_episode_steps=500)
class PickMeatFromPotEnv(BaseEnv):
    """
    Two robots task for picking meat from pot with active perception.
    
    Robot 0: Active perception (look inside the pot with wrist camera)
    Robot 1: Pick meat and place to goal region
    """
    ...
    def __init__(
        self, *args, robot_uids=("panda_wristcam", "panda_wristcam"), robot_init_qpos_noise=0.02, **kwargs
    ):
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'pick_meat_from_pot.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        # Align meat position with pot body position
        pot_pose = self.pot_body.pose.p
        
        # Position meat inside pot body
        meat_pose = self.meat.pose
        meat_pose.p[:, 0] = pot_pose[:, 0]
        meat_pose.p[:, 1] = pot_pose[:, 1]
        self.meat.set_pose(meat_pose)

    @property
    def perception_agent(self) -> PandaWristCam:
        """Robot 0: Active perception (look inside the pot)"""
        return self.agent.agents[0]

    @property
    def picker_agent(self) -> PandaWristCam:
        """Robot 1: Pick meat and place to goal"""
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode:
            obs.update(
                ...
                pot_body_pose=self.pot_body.pose.raw_pose,
                meat_pose=self.meat.pose.raw_pose,
                perception_tcp_to_pot=self.pot_body.pose.p - self.perception_agent.tcp.pose.p,
