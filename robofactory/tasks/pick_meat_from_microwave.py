...
@register_env("PickMeatFromMicrowave-rf", max_episode_steps=500)
class PickMeatFromMicrowaveEnv(BaseEnv):
    """
    Three robots task for picking meat from microwave with active perception.
    ...
    def __init__(
        ...
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'pick_meat_from_microwave.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
