...
@register_env("StackCubeActive-rf", max_episode_steps=200)
class StackCubeActiveEnv(BaseEnv):
    SUPPORTED_ROBOTS = ["panda_wristcam", "panda", "fetch"]
    agent: Union[MultiAgent[Tuple[Panda, Panda]]]
    ...
    def __init__(
        ...
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'stack_cube_active.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
