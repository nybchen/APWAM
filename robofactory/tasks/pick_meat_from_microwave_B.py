...
@register_env("PickMeatFromMicrowaveB-rf", max_episode_steps=500)
class PickMeatFromMicrowaveBEnv(BaseEnv):
    """
    Three robots task for picking meat from microwave with active perception - Mode B.
    ...
    def __init__(
        ...
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'pick_meat_from_microwave_B.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
