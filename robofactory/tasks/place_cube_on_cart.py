...
@register_env("PlaceCubeOnCart-rf", max_episode_steps=500)
class PlaceCubeOnCartEnv(BaseEnv):
    """
    Two robots task for placing a cube on a cart with active perception.

    Robot 0: Active perception (look at the whole scene from top)
    Robot 1: Pick the cube from the table and place it onto the cart
    """
    ...
    def __init__(
        ...
        if 'config' in kwargs:
            ...
            if 'scene' in kwargs:
                ...
                scene = 'table'
            with open(osp.join(CONFIG_DIR, scene, 'place_cube_on_cart.yaml'), 'r', encoding='utf-8') as f:
                cfg = yaml.load(f.read(), Loader=yaml.FullLoader)
    ...
    def picker_agent(self) -> PandaWristCam:
        """Robot 1: Pick cube and place on cart"""
        return self.agent.agents[1]

    def evaluate(self):
        cube_pose = self.cube.pose.p
        goal_pose = self.goal_region_cart.pose.p

        # Check if cube is placed in goal region on cart (2D distance in xy)
        cube_to_goal_dist = torch.linalg.norm(
            cube_pose[:, :2] - goal_pose[:, :2], axis=1
        )
        cube_in_goal = cube_to_goal_dist < self.goal_radius
        success = cube_in_goal.bool()
        return {
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        if "state" in self.obs_mode:
            obs.update(
                goal_region_cart_pos=self.goal_region_cart.pose.p,
                cart_pose=self.cart.pose.raw_pose,
                cube_pose=self.cube.pose.raw_pose,
                picker_tcp_to_cube=self.cube.pose.p - self.picker_agent.tcp.pose.p,
                cube_to_goal_cart=self.goal_region_cart.pose.p - self.cube.pose.p,
            )
        return obs
